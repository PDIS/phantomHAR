...
var ALLOWED_CONTENT_TYPES = ['css', 'js', 'json', 'doc', 'cssimage'];
      ...
      request(reqOpts).on('response', function (res) {
        ...
        function tally() {
          ...
          if (options.bodies && ALLOWED_CONTENT_TYPES.indexOf(entry.response.content._type) !== -1) {
            ...
            entry.response.content.text = new Buffer(body, 'binary').toString();
          }
          if (entry.response.content._type === 'cssimage') {
            entry.response.content.text = new Buffer(body, 'binary').toString('base64');
          }
